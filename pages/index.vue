...
<template>
  <div class="overflow-x-hidden"> 
    <section id="section1" class="min-h-screen pt-24 transition-colors duration-300 bg-zinc-800 dark:bg-white text-white w-full">
      <ul>
        <li v-for="post in posts" :key="post.id" class="mb-8">
          <div class="flex flex-col md:grid md:grid-cols-2 mt-6">
            <div class="col-start-1">
              <h1 class="text-2xl md:text-9xl text-white dark:text-black ml-12 font-bold font-arvo">
                {{ post.title.rendered }}
              </h1>
    ...
    <section id="section2" class="min-h-screen pt-24 pb-14 transition-colors duration-300 bg-zinc-800 dark:bg-white text-white w-full">
      <div class="ml-14 md:ml-36 gap-2 md:gap-4"> 
        <div class="text-xl md:text-7xl font-semibold text-white dark:text-black tracking-tighter font-arvo">
          HELLO I AM GENO
        </div>
        <div class="row-start-2 text-lg md:text-xl font-semibold text-white dark:text-black font-chakra md:pl-[37%]">
          Kooijman
        </div>
        <div class="pl-1 md:pl-12 text-lg md:text-3xl text-white dark:text-black font-chakra w-[90%] md:w-[40%]">
          {{ customFieldAbout }}
        </div>
      </div>
    </section>
      ...
      <main id="content" class="site-content w-full" role="main">
        <section id="panels">
          <div id="panels-container" class="flex w-full overflow-hidden"> 
            <section id="section3" class="panel flex-shrink-0 flex-grow-0 w-screen min-h-screen pt-24 pb-14 transition-colors duration-300 bg-zinc-800 dark:bg-white text-white">
              <div v-if="firstProject" class="flex justify-center mb-8">
                <img class="w-[250px] h-[300px]" :src="firstProject.image" :alt="firstProject.title" v-if="firstProject.image" />
                <p v-else>No image available</p>
              </div>
            </section>

            <section id="section4" class="panel flex-shrink-0 flex-grow-0 w-screen min-h-screen pt-24 pb-14 transition-colors duration-300 bg-zinc-800 dark:bg-white text-white">
              <div v-if="thirdProject" class="flex justify-center mb-8">
                <img class="w-[250px] h-[300px]" :src="thirdProject.image" :alt="thirdProject.title" v-if="thirdProject.image" />
                <p v-else>No image available</p>
              </div>
            </section>

            <section id="section5" class="panel flex-shrink-0 flex-grow-0 w-screen min-h-screen pt-24 pb-14 transition-colors duration-300 bg-zinc-800 dark:bg-white text-white">
              <div v-if="fourthProject" class="flex justify-center mb-8">
                <img class="w-[250px] h-[300px]" :src="fourthProject.image" :alt="fourthProject.title" v-if="fourthProject.image" />
                <p v-else>No image available</p>
              </div>
            </section>
          </div>
    ...
    <section id="section6" class="min-h-screen w-full pt-24 pb-14 transition-colors duration-300 bg-zinc-800 dark:bg-white text-white">
      <div v-if="fourthProject" class="flex justify-center mb-8">
        <img class="w-[250px] h-[300px]" :src="fourthProject.image" :alt="fourthProject.title" v-if="fourthProject.image" />
        <p v-else>No image available</p>
      </div>
